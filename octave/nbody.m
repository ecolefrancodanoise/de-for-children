...
radius= .7*52.9e-12; % bohr radius, pm times adjustment factor
coulombCoeff = 9e9;
x = [x0 x0; -x0 -x0; x0+radius x0+radius; -x0-radius -x0-radius]
xp = [0 0; 0 0; 1 -1; -1 1] * 2.19e6 * 0.85; %electron speed times adjustment factor
m= [1.7e-24 1.7e-24 9e-31 9e-31]';
charges = 1.6e-19*[1 1 -1 -1]';
...
hold off
plot(xresult(1, :, 1), xresult(2, :, 1));
hold on
plot(xresult(1, :, 2), xresult(2, :, 2));
